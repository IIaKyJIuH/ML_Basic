            ...
            if best_split:
                ...
                if len(left_dataset[1]) and len(right_dataset[1]):
                    ...
                    left_subtree, l_cnt = self._build_tree(*left_dataset, cur_depth=cur_depth + 1,
                                                           leaves_available=leaves_available - 1)
                    # recur right
...
class MyForest(ABC):
    ...
    def _predict_oob(self, estimator: MyTree, oob_dict: Dict[Hashable, List[float]], data: pd.DataFrame):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series):
        ...
        for _ in range(self.n_estimators):
            cols_idx = random.sample(range(len(X.columns)), cols_smpl_cnt)
            rows_idx = random.sample(range(len(X)), rows_smpl_cnt)
            estimator = self._TreeClass(**{param: vars(self).get(param, default)
                                           for param, default in self._tree_params.items()})
            oob_data = X.drop(rows_idx, axis=0).iloc[:, cols_idx]
            sub_X = X.iloc[rows_idx, cols_idx]
            sub_y = y.loc[sub_X.index]
            estimator.fit(sub_X, sub_y, len(X))
            self.leafs_cnt += estimator.leafs_cnt
