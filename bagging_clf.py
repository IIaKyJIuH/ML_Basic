...
class MyBagging(ABC):
    def __init__(self, estimator: Optional[Union[MyLinear, MyKNN, MyTree]] = None, n_estimators: int = 10,
        ...
        if oob_score not in ['entropy', 'gini']:
...
class MyBaggingClf(MyBagging):
    def __init__(self, estimator: Optional[Union[MyLinear, MyKNN, MyTree]] = None, n_estimators: int = 10,
                 max_samples: float = 1.0, random_state: int = 42, oob_score: Optional[str] = 'entropy'):
    ...
    def predict(self, X: pd.DataFrame, type: str = 'mean'):
        if type == 'mean':
            ...
            results = []
            for estimator in self.estimators:
                results.append(estimator.predict(X).astype(int))
            return ((reduce(lambda x, y: x + y, results) / len(results)) >= 0.5).astype(int)

    def _predict_oob(self, estimator: Union[MyLogReg, MyKNNClf, MyTreeClf], oob_dict: Dict[Hashable, List[float]],
                     data: pd.DataFrame):
