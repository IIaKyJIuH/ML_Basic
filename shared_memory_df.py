...
class SharedPandasSeries:
    '''
    Wraps a pandas series so that it can be shared quickly among processes,
    avoiding unnecessary copying and (de)serializing.
    '''
    def __init__(self, srs: pd.Series):
        '''
        Creates the shared memory and copies the series therein
        '''
        self._values = SharedNumpyArray(srs.values)
        self._index = srs.index

    def read(self):
        '''
        Reads the series from the shared memory
        without unnecessary copying.
        '''
        return pd.Series(
            self._values.read(),
    ...
    def copy(self):
        '''
        Returns a new copy of the series stored in shared memory.
        '''
        return pd.Series(
            self._values.copy(),
            index=self._index
